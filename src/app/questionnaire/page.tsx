  ...
  if (userLoading) {
    return (
      <div className="app-container justify-center items-center">
        <Hourglass className="text-[#1E1E1E] w-32 h-32 mx-auto animate-pulse" />
          ...
          <div className="space-y-3">
            {questions[currentQuestion].options.map((option, index) => (
              <Button
                key={option}
                variant="outline"
                className="w-full justify-start text-oraculo-dark hover:text-[#1E1E1E] hover:bg-[#1E1E1E]/10 h-auto py-4 px-4 rounded-xl transition-all duration-200 hover:scale-[1.02]"
                onClick={() => handleAnswer(option)}
              >
                <span className="mr-3 bg-[#1E1E1E]/10 w-8 h-8 rounded-full flex items-center justify-center text-[#1E1E1E] font-semibold">
                  {index + 1}
                </span>
